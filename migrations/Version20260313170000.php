...
final class Version20260313170000 extends AbstractMigration
{
    ...
    {
        if (!$this->tableExists('patient_caregiver')) {
        ...
        if (!$this->foreignKeyExists('patient_caregiver', 'FK_8D1A82336B899279')) {
        ...
        if (!$this->foreignKeyExists('patient_caregiver', 'FK_8D1A8233A76ED395')) {
        ...
        if ($this->tableExists('care_team_patient') && $this->tableExists('care_team_member')) {
        ...
        if ($this->tableExists('operation_chirurgien') && $this->tableExists('operation') && $this->tableExists('Chirurgien')) {
        ...
        if ($this->tableExists('operation_infirmiere') && $this->tableExists('operation') && $this->tableExists('infirmiere')) {
        ...
        $this->addSql('DROP TABLE IF EXISTS care_team_patient');
        $this->addSql('DROP TABLE IF EXISTS care_team_member');
        $this->addSql('DROP TABLE IF EXISTS care_team');
    }

    public function down(Schema $schema): void
    {
        if (!$this->tableExists('care_team')) {
        ...
        if (!$this->tableExists('care_team_member')) {
        ...
        if (!$this->tableExists('care_team_patient')) {
        ...
        if (!$this->foreignKeyExists('care_team_member', 'FK_AEAE93891526159E')) {
        ...
        if (!$this->foreignKeyExists('care_team_member', 'FK_AEAE9389A76ED395')) {
        ...
        if (!$this->foreignKeyExists('care_team_patient', 'FK_7783DD0A1526159E')) {
        ...
        if (!$this->foreignKeyExists('care_team_patient', 'FK_7783DD0A6B899279')) {
        ...
        if ($this->tableExists('Patient')) {
            $this->addSql("INSERT IGNORE INTO care_team (id, name, description) SELECT p.id, CONCAT('Équipe ', p.Name, ' ', p.FirstName), NULL FROM Patient p");
            $this->addSql('INSERT IGNORE INTO care_team_patient (care_team_id, patient_id) SELECT p.id, p.id FROM Patient p');
        }

        if ($this->tableExists('patient_caregiver')) {
            $this->addSql('INSERT IGNORE INTO care_team_member (care_team_id, user_id) SELECT pc.patient_id, pc.user_id FROM patient_caregiver pc');
            $this->addSql('DROP TABLE IF EXISTS patient_caregiver');
        }
    }
